...
// 建立对象
extern strnew New_Str_Obj(const void *Master, int SizeNum,
                          int SizeType); // 建立对象的函数
...
/*-----------------------------------外部接口----------------------------------*/
extern int catString(char *OutStr, const char *IntStr, int MaxSize,
                     int IntSize);
extern bool copyString(char *OutStr, const char *IntStr, int MaxSize,
                       int IntSize);
extern char *myStrstr(char *MotherStr, char *SonStr, int MotherMaxSize);
extern char *myStrstrCont(char *MotherStr, char *SonStr, int MotherMaxSize,
                          int ContNum);
...
#define newType(...)                                                           \
  GET_TYPE_MACRO(__VA_ARGS__, newType2, newType1)(__VA_ARGS__)
#endif
