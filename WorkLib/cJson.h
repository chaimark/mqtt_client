...
typedef struct _JsonArray {
  ...
  void (*get)(struct _JsonArray This, strnew OutStr,
              int ItemNum); // 获取Json 某元素
  void (*getArray)(struct _JsonArray This, strnew OutStr,
                   int ItemNum); // 获取Json 某元素
} JsonArray;
// 建立对象示例
...
typedef struct _JsonObject {
  ...
  signed char (*isJsonNull)(struct _JsonObject This,
                            char Key[]);              // 判断Json 是否为空
  ...
  void (*getString)(struct _JsonObject This, char Key[],
                    strnew OutStr); // 获取Json string
  struct _JsonArray (*getArray)(struct _JsonObject This, char Key[],
                                strnew OutStr); // 获取Json array
  struct _JsonObject (*getObject)(struct _JsonObject This, char Key[],
                                  strnew OutStr); // 获取Json object
} JsonObject;
// 建立对象示例
...
#define newClassJsonItem(JsonStringSpace, name, code, ClassType_1,             \
                         ClassType_2)                                          \
  do {                                                                         \
    if (strlen(name) == 0) {                                                   \
    ...
    code;                                                                      \
    catString(JsonStringSpace.Name._char, ClassType_2, JsonStringSpace.MaxLen, \
              1);                                                              \
    char *lastAddr =                                                           \
        &JsonStringSpace.Name._char[strlen(JsonStringSpace.Name._char)];       \
    swapChr((lastAddr - 1), (lastAddr - 2));                                   \
  } while (0)

#define newSubObjectJsonItem(JsonStringSpace, name, code)                      \
  newClassJsonItem(JsonStringSpace, name, code, "{", "}")
#define newSubArrayJsonItem(JsonStringSpace, name, code)                       \
  newClassJsonItem(JsonStringSpace, name, code, "[", "]")
