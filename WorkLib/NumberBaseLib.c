...
// 中需要转换的长度)
void numberArrayToStrArray(strnew StrArray, const strnew NumberArray,
                           int NeedLen) {
...
// 中需要转换的长度)
void strArrayToNumberArray(strnew NumberArray, const strnew StrArray,
                           int NeedLen) {
...
// 任意进制数 转 任意进制数组
int anyBaseNumberToAnyBaseArray(uint64_t Number, int IntputBase, int OutputBase,
                                strnew OutArray) {
...
int64_t anyBaseArrayToAnyBaseNumber(strnew IntArray, int IntputBase,
                                    int OutputBase) {
  swapStr(IntArray.Name._char, IntArray.MaxLen); // 先将数组从从大端模式改为小端
  int64_t TempNum =
      anyArrayToSameBaseNumber((uint8_t *)IntArray.Name._char, IntArray.MaxLen);
  ...
  if (&(inputArray.Name._char) != &(OutputArray.Name._char)) {
    ...
    for (ResLen = 0; ResLen < (inputArray.MaxLen / 2); ResLen++) {
      OutputArray.Name._char[ResLen] =
          ((inputArray.Name._char[ResLen * 2] & 0x0F) << 4) |
...
int doneAsciiStrToAnyBaseNumberData(const char AscArray[], int OutputBase) {
  ...
  strArrayToNumberArray(NumStr, (const strnew)String_temp,
                        NumStrNowLen); // 去掉 0x30
  int NumTemp =
      anyBaseArrayToAnyBaseNumber(NumStr, 10, OutputBase); // 组合成 任意进制数
  return ((sign == 1) ? NumTemp : -NumTemp);
}
// 任意进制数 转 字符串
int doneBaseNumberDataToAsciiStr(strnew AscArray, int NumberData,
                                 int IntputBase) {
  uint64_t TempNum = anyBaseToAnyBase(NumberData, IntputBase, 10);
  strnew_malloc(TempArr, 16);
  int AscArrayLen =
      anyBaseNumberToSameArray(TempArr, TempNum); // 10进制 转对应数组
  numberArrayToStrArray(AscArray, TempArr, AscArrayLen); // 数组串 转 字符串
  return AscArrayLen;
}

// 字符串 转 double
double doneAsciiToDouble(char AscArray[]) { return atof(AscArray); }
// double 转 字符串
...
int ASCIIToHEX2(const strnew asc, strnew hex) {
  ...
  int Len = doubleChStrToShortChStr(
      temp_hex, hex); // 双字节数组 转 单字节数组 0x02 0x03 --> 0x23
  return Len;
  // STR TO HEX-------------------
}
...
// 获取某个值在某段区间类所在点的百分比
float getPartOfSetPointOnRing(uint32_t SetPoint, uint32_t Min_Ring,
                              uint32_t Max_Ring) {
