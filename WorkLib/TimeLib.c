...
// 计算时间戳(秒数)
uint32_t get_timestamp(uint32_t NowYear, uint32_t NowMonth, uint32_t NowDay,
                       uint32_t NowHour, uint32_t NowMinute,
                       uint32_t NowSecond) {
  // 计算从1970年1月1日到指定日期的总天数
...
uint32_t getTimeNumber_UTCByRTCTime(strnew RTCTime_String) {
  ...
  sscanf(RTCTime_String.Name._char, "%d-%d-%d %d:%d:%d", &year, &month, &day,
         &hour, &min, &sec);
...
void DelayUs_General(uint32_t Delay) {
  ...
  while (1) {
    ...
    uint32_t CountTickDelta =
        (NowVAL < StartTick ? StartTick - NowVAL
                            : (uint32_t)SysTick->LOAD - (NowVAL - StartTick));
