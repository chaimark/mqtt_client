...
static bool getFromTypeCheckDoubleOrFloat(strnew FromStr) {
  ...
  do {
    ...
    // 找到格式字符串里的第一个 %
    if ((getNowType(NowAddr, UserFromNow) == 'f') ||
        (getNowType(NowAddr, UserFromNow) == 'F')) {
      return true;
    }
...
static bool isNeedBySignDivde(strnew InputString, int Addr_Over) {
  ...
  for (int i = Addr_Over; i > 0; i--) {
    ...
    if ((ReFlag > 0) && ((InputString.Name._char[i] == '}') ||
                         (InputString.Name._char[i] == ']'))) {
      ReFlag--;
      continue;
    }
    if (ReFlag < 1) {
...
void addJsonItemData(strnew JsonStringSpace, const char *FromStr, ...) {
  ...
  if (Addr_OverName != NULL) {
    ...
    memcpy(KeyName, Addr_Start,
           (strlen(Addr_Start) < 200 ? strlen(Addr_Start) : 200));
  }
  ...
  va_end(args);
#ifdef IsOpenFloatHelp_Ability
  if (getFromTypeCheckDoubleOrFloat(NEW_NAME(KeyName)) ||
      getFromTypeCheckDoubleOrFloat(NEW_NAME(KeyName))) {
...
// 查找双重字符位置
static char *getDoubleChrOnString(char *MotherString, char HeadChr,
                                  char EndChr) {
  Stack s;
  initStack(&s);
  char *result = NULL;
...
static int Arr_sizeItemNum(struct _JsonArray This) {
  ...
  if (myStrstrCont(This.JsonString.Name._char, "\"", This.JsonString.MaxLen,
                   2) != NULL) {
    isStringArray = true;
  }
  while (*EndItem != '\0') {
...
static signed char Arr_isJsonNull(struct _JsonArray This) {
  ...
  int i = 0;
  while (!(StartP[i] != ' ' && StartP[i] != '\t') && (StartP[i] != '\0') &&
         (StartP[i] != ']')) {
    i++;
  }
...
static void Arr_get(struct _JsonArray This, strnew OutStr, int ItemNum) {
  ...
  *(EndItem - 1) = Temp;
  ...
  for (int i = 0; i < NowLineMaxLen; i++) {
    if ((OutStr.Name._char[i] != '\0') && (OutStr.Name._char[i] != '\t') &&
        (OutStr.Name._char[i] != '[') && (OutStr.Name._char[i] != ' ') &&
...
static signed char Obj_isJsonNull(struct _JsonObject This, char Key[]) {
  ...
  if ((KeyP = strstr(This.JsonString.Name._char, SonStr)) != NULL) {
    ...
    char Temp = *(KeyP + 4);
    *(KeyP + 4) = '\0';
    if ((strcmp(KeyP, "null") == 0) || (strcmp(KeyP, "NULL") == 0) ||
        (strcmp(KeyP, "Null") == 0)) {
      ResOver = true;
    }
    *(KeyP + 4) = Temp;
...
static bool Obj_getBool(struct _JsonObject This, char Key[]) {
  ...
  if ((KeyP = strstr(This.JsonString.Name._char, SonStr)) != NULL) {
    ...
    char Temp = *(KeyP + 4);
    *(KeyP + 4) = '\0';
    if ((strcmp(KeyP, "true") == 0) || (strcmp(KeyP, "TRUE") == 0) ||
        (strcmp(KeyP, "True") == 0)) {
      ResBool = true;
    } else if ((strcmp(KeyP, "false") == 0) || (strcmp(KeyP, "FALSE") == 0) ||
               (strcmp(KeyP, "False") == 0)) {
      ResBool = false;
    }
    *(KeyP + 4) = Temp;
...
static struct _JsonArray Obj_getArray(struct _JsonObject This, char Key[],
                                      strnew OutStr) {
  ...
  if ((KeyP = strstr(This.JsonString.Name._char, SonStr)) != NULL) {
    ...
    if ((EndP = getDoubleChrOnString(KeyP, '[', ']')) != NULL) {
      ...
      *(EndP + 1) = '\0';
      if (OutStr.Name._char != This.JsonString.Name._char) {
        memset(tempJsonArr.JsonString.Name._char, 0,
               tempJsonArr.JsonString.MaxLen);
        copyString(tempJsonArr.JsonString.Name._char, KeyP,
...
// 注意输出地址与原json字符串地址一致时, 会破坏原数据
static struct _JsonObject Obj_getObject(struct _JsonObject This, char Key[],
                                        strnew OutStr) {
  ...
  if ((KeyP = strstr(This.JsonString.Name._char, SonStr)) != NULL) {
    ...
    if ((EndP = getDoubleChrOnString(KeyP, '{', '}')) != NULL) {
      ...
      *(EndP + 1) = '\0';
      if (OutStr.Name._char != This.JsonString.Name._char) {
        memset(tempJsonObj.JsonString.Name._char, 0,
               tempJsonObj.JsonString.MaxLen);
        copyString(tempJsonObj.JsonString.Name._char, KeyP,
...
void addCsToJsonAndPushJsonStr(JsonObject InputJsonStrObj) {
  ...
  if ((AddrOver + (int)strlen(",\"NowCheckNum\":xxxxx") >
       InputJsonStrObj.JsonString.MaxLen) ||
      (AddrOver < 1)) {
...
bool checkOfCsJsonStrIsRight(strnew JsonInputStr, strnew JsonOutputStr) {
  ...
  if ((PEnd == NULL) ||
      (PEnd >= JsonObj.JsonString.Name._char + JsonObj.JsonString.MaxLen - 1)) {
    return false;
  }
  (*(PEnd++)) = '}';
  (*PEnd) = '\0';
  if (JsonInputStr.Name._char != JsonOutputStr.Name._char) {
    copyString(JsonOutputStr.Name._char, JsonInputStr.Name._char,
               JsonOutputStr.MaxLen, JsonInputStr.MaxLen);
  }
  int DataLen = strlen(JsonInputStr.Name._char);
  JsonInputStr.Name._char[DataLen++] =
      (((uint16_t)(NowCheckNum) & 0xFF00) >> 8);
  JsonInputStr.Name._char[DataLen++] =
      (((uint16_t)(NowCheckNum) & 0x00FF) >> 0);
  // 计算 cs 或其他检验算法
