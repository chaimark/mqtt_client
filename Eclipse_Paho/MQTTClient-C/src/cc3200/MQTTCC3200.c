...
void SysTickIntHandler(void) { MilliTimer++; }
...
void InitTimer(Timer *timer) { timer->end_time = 0; }
...
void cc3200_disconnect(Network *n) { sl_Close(n->my_socket); }
...
int TLSConnectNetwork(Network *n, char *addr, int port,
  ...
  retVal = sl_SetSockOpt(n->my_socket, SL_SOL_SOCKET, SL_SO_SECMETHOD, &method,
                         sizeof(method));
  if (retVal < 0) {
    return retVal;
  }
  ...
  retVal = sl_SetSockOpt(n->my_socket, SL_SOL_SOCKET, SL_SO_SECURE_MASK, &mask,
                         sizeof(mask));
  if (retVal < 0) {
    return retVal;
  }
  ...
  if (certificates != NULL) {
    retVal =
        sl_SetSockOpt(n->my_socket, SL_SOL_SOCKET, SL_SO_SECURE_FILES,
