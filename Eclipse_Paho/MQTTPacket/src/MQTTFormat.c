...
int MQTTStringFormat_connect(char *strbuf, int strbuflen,
  ...
  if (data->willFlag)
    strindex += snprintf(
        ...
        data->will.qos, data->will.retained, data->will.topicName.lenstring.len,
        data->will.topicName.lenstring.data, data->will.message.lenstring.len,
        data->will.message.lenstring.data);
  if (data->username.lenstring.data && data->username.lenstring.len > 0)
    strindex +=
        snprintf(&strbuf[strindex], strbuflen - strindex, ", user name %.*s",
                 data->username.lenstring.len, data->username.lenstring.data);
  if (data->password.lenstring.data && data->password.lenstring.len > 0)
    strindex +=
        snprintf(&strbuf[strindex], strbuflen - strindex, ", password %.*s",
                 data->password.lenstring.len, data->password.lenstring.data);
  return strindex;
}

int MQTTStringFormat_connack(char *strbuf, int strbuflen,
...
int MQTTStringFormat_publish(char *strbuf, int strbuflen, unsigned char dup,
...
int MQTTStringFormat_suback(char *strbuf, int strbuflen,
                            unsigned short packetid, int count,
                            int *grantedQoSs) {
  return snprintf(strbuf, strbuflen,
...
char *MQTTFormat_toClientString(char *strbuf, int strbuflen, unsigned char *buf,
                                int buflen) {
  int index = 0;
  int rem_length = 0;
  MQTTHeader header = {0};
  ...
  case PUBLISH: {
    ...
    if (MQTTDeserialize_publish(&dup, &qos, &retained, &packetid, &topicName,
      ...
      strindex =
          MQTTStringFormat_publish(strbuf, strbuflen, dup, qos, retained,
  ...
  case SUBACK: {
    ...
    int grantedQoSs[1];
    if (MQTTDeserialize_suback(&packetid, maxcount, &count, grantedQoSs, buf,
                               buflen) == 1)
      strindex = MQTTStringFormat_suback(strbuf, strbuflen, packetid, count,
                                         grantedQoSs);
  } break;
  case UNSUBACK: {
    unsigned short packetid;
...
char *MQTTFormat_toServerString(char *strbuf, int strbuflen, unsigned char *buf,
                                int buflen) {
  int index = 0;
  int rem_length = 0;
  MQTTHeader header = {0};
  ...
  case SUBSCRIBE: {
    ...
    if (MQTTDeserialize_subscribe(&dup, &packetid, maxcount, &count,
                                  ...
                                  buflen) == 1)
      strindex = MQTTStringFormat_subscribe(strbuf, strbuflen, dup, packetid,
  ...
  case UNSUBSCRIBE: {
    ...
    if (MQTTDeserialize_unsubscribe(&dup, &packetid, maxcount, &count,
      ...
      strindex = MQTTStringFormat_unsubscribe(strbuf, strbuflen, dup, packetid,
                                              count, topicFilters);
  } break;
  case PINGREQ:
  case PINGRESP:
